...
$.fn.answerSheet = function(options) {var defaults={mold:'card',};
...
return $(this).each(function()
    ...
    {
    	obj.eq(_b).css({'z-index':a});_b-=1;}$(this).show();
   if(opts.mold == 'card')
   {
      ...
      if(_idx == _len){return;}
    	    ...
    	    setTimeout(function()
    		  {
    	        _cardcont.addClass('cardn');setTimeout(function()
    		   {
    	         _cards.eq(_idx + 3).addClass('card3');_cards.eq(_idx + 2).removeClass('card3').addClass('card2');
                 _cards.eq(_idx + 1).removeClass('card2').addClass('card1');_cardcont.removeClass('card1');
               },200);
    	      },100);
    	   }
        });
//上一页
  $('.card_bottom').find('.prev').click(function()
       ...
       setTimeout(function()
    		   {obj.eq(_idx - 1).addClass('card1').removeClass('cardn');},200);
        })
      
   //下一页  
  $('.card_bottom1').find('.next').click(function()
			{
	     var _idx =  $(this).parents(_cont).index(),_cards =  obj,_cardcont = $(this).parents(_cont);
   if(_idx == _len){return;}
   else{
